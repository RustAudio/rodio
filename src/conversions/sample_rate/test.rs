        ...
        fn upsample() {
            ...
            let output =
                SampleRateConverter::new(input.into_iter(), SampleRate(2000), SampleRate(3000), 2)
                    .map(|s| s as u16);
            assert_eq!(output.size_hint().0, 12); // Test the source's Iterator::size_hint()
        ...
        fn upsample2() {
            ...
            let output =
                SampleRateConverter::new(input.into_iter(), SampleRate(1000), SampleRate(7000), 1)
                    .map(|s| s as u16);
            let (lower_bound, upper_bound) = output.size_hint();
            let output = output.collect::<Vec<_>>();
            assert_eq!(output, [1, 2, 4, 6, 8, 10, 12, 14]);

            assert!((lower_bound as f32 / output.len() as f32).abs() < 2.0);
            assert!((upper_bound.unwrap() as f32 / output.len() as f32).abs() < 2.0);
        }
...
// mod fast_inhouse {
//     use super::super::fast_inhouse::SampleRateConverter;
//     use core::time::Duration;
//     use cpal::{ChannelCount, SampleRate};
//     use quickcheck::TestResult;
//     use quickcheck_macros::quickcheck;
//
//     test_resampler!(SampleRateConverter);
// }

mod hifi_rubato {
    use super::super::hifi_rubato::SampleRateConverter;
    use core::time::Duration;
    use cpal::{ChannelCount, SampleRate};
    use quickcheck::TestResult;
    use quickcheck_macros::quickcheck;
